...
import { getRoles, createRole, updateRole, deleteRole } from "@/lib/api/masters";
...
export default function RolesPage() {
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createRole(form.code, form.name);
      setIsAddOpen(false);
      loadData();
    } catch (err: unknown) {
      const e = err as { response?: { data?: { error?: { message?: string } } } };
      setFormError(e?.response?.data?.error?.message || "Failed to create role.");
    } finally { setIsSaving(false); }
  };

  const handleEditSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedRole || !form.code || !form.name) { setFormError("All fields are required."); return; }
    setIsSaving(true); setFormError(null);
    try {
      await updateRole(selectedRole.id, form.code, form.name);
      setIsEditOpen(false);
      loadData();
    } catch (err: unknown) {
      const e = err as { response?: { data?: { error?: { message?: string } } } };
      setFormError(e?.response?.data?.error?.message || "Failed to update role.");
    } finally { setIsSaving(false); }
  };
    ...
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Manage permission levels, access rules, and department roles.</p>
        </div>
        ...
        <table className="w-full border-collapse text-left text-sm">
          ...
          <tbody className="divide-y divide-border">
            {isLoading ? (
              ...
              <tr><td colSpan={3} className="px-6 py-12 text-center text-muted-foreground">No roles created yet.</td></tr>
            ) : (
              roles.map((role) => (
                <tr key={role.id} className="hover:bg-slate-500/5 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => handleOpenEdit(role)} className="p-1.5 rounded-lg text-muted-foreground hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-foreground transition-colors"><Edit2 className="h-4 w-4" /></button>
                      <button onClick={() => handleDelete(role.id)} className="p-1.5 rounded-lg text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20 transition-colors"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </td>
                </tr>
              ))
      ...
      <Dialog isOpen={isAddOpen} onClose={() => setIsAddOpen(false)} title="Add Role" size="sm">
        <form onSubmit={handleAddSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <input type="text" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} placeholder="e.g. MGR" className="w-full px-3 py-2 rounded-lg bg-background border border-input text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-ring" required />
          </div>
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-muted-foreground">Role Name *</label>
            <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Manager" className="w-full px-3 py-2 rounded-lg bg-background border border-input text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-ring" required />
          </div>
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-border">
            ...
            <button type="submit" disabled={isSaving} className="px-4 py-2 bg-brand-500 hover:bg-brand-600 disabled:opacity-60 text-white rounded-lg text-sm font-medium transition-colors">{isSaving ? "Saving..." : "Save Role"}</button>
          </div>
        </form>
      </Dialog>
      ...
      <Dialog isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} title="Edit Role" size="sm">
        <form onSubmit={handleEditSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-border">
            ...
            <button type="submit" disabled={isSaving} className="px-4 py-2 bg-brand-500 hover:bg-brand-600 disabled:opacity-60 text-white rounded-lg text-sm font-medium transition-colors">{isSaving ? "Saving..." : "Update Role"}</button>
          </div>
        </form>
      </Dialog>
    </div>

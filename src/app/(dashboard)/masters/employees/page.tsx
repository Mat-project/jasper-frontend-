...
export default function EmployeesPage() {
  ...
  const handleToggleStatus = (id: string) => {
    mockService.toggleEmployeeStatus(id);
    loadData();
  };
  ...
  const handleOpenEdit = (emp: User) => {
    ...
    setForm({
      ...
      department: (emp as any).department || departments[0]?.name || "",
      section: (emp as any).section || sections[0]?.name || "",
  ...
  const handleOpenDetails = (emp: User) => {
    setSelectedEmp(emp);
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
    ...
    try {
      mockService.createEmployee(form);
      setIsAddOpen(false);
      loadData();
    } catch (err: any) {
  ...
  const handleEditSubmit = (e: React.FormEvent) => {
    ...
    try {
      mockService.updateEmployee(selectedEmp.id, form);
      setIsEditOpen(false);
      loadData();
    } catch (err: any) {
  ...
  const filteredEmployees = employees.filter((emp) => {
    const code = ((emp as any).employee_code || "").toLowerCase();
    const name = (emp.full_name || "").toLowerCase();
    const email = (emp.email || "").toLowerCase();
    const query = search.toLowerCase();

    const matchesSearch = code.includes(query) || name.includes(query) || email.includes(query);

    const matchesDept = selectedDept === "all" || (emp as any).department === selectedDept;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage your organization&apos;s engineering staff, department tags, and access permissions.
          </p>
        </div>
        <button
              ...
              filteredEmployees.map((emp) => (
                <tr key={emp.id} className="hover:bg-slate-500/5 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="text-foreground">{(emp as any).department || "—"}</div>
                    <div className="text-xs text-muted-foreground">{(emp as any).section || "—"}</div>
                  </td>
                  <td className="px-6 py-4">
      ...
      <Dialog isOpen={isAddOpen} onClose={() => setIsAddOpen(false)} title="Add New Employee" size="md">
        <form onSubmit={handleAddSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="space-y-1.5">
              ...
              <select
                ...
                className="w-full px-2 py-2 rounded-lg bg-background border border-input text-sm text-foreground focus:outline-none"
              >
                {departments.map((d) => (
                  <option key={d.id} value={d.name}>
                    {d.name}
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-border">
            ...
            <button
              type="submit"
              className="px-4 py-2 bg-brand-500 hover:bg-brand-600 text-white rounded-lg text-sm font-medium shadow-md transition-colors"
            >
              Save Employee
            </button>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <span className="text-xs text-muted-foreground font-mono">
                  {(selectedEmp as any).employee_code || "—"}
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-y-4 gap-x-2 text-sm border-t border-border pt-4">
              <div>
                ...
                <div className="text-foreground font-medium mt-0.5">{selectedEmp.email}</div>
              </div>
              <div>
                <div className="text-xs font-semibold text-muted-foreground">Phone</div>
                <div className="text-foreground font-medium mt-0.5">{selectedEmp.phone_number || "—"}</div>
              </div>
              <div>
                <div className="text-xs font-semibold text-muted-foreground">Department</div>
                <div className="text-foreground font-medium mt-0.5">{(selectedEmp as any).department || "—"}</div>
              </div>
              <div>
                <div className="text-xs font-semibold text-muted-foreground">Section</div>
                <div className="text-foreground font-medium mt-0.5">{(selectedEmp as any).section || "—"}</div>
              </div>
              <div>
                <div className="text-xs font-semibold text-muted-foreground">Role</div>
                <div className="text-foreground font-medium mt-0.5">{selectedEmp.roles[0]?.name || "Viewer"}</div>
              </div>
              <div>
              ...
              <div className="col-span-2">
                <div className="text-xs font-semibold text-muted-foreground">Date Joined</div>
                <div className="text-foreground font-medium mt-0.5">
                  {new Date(selectedEmp.date_joined).toLocaleDateString("en-US", {
                    year: "numeric",
                    month: "long",
                    day: "numeric",
                  })}

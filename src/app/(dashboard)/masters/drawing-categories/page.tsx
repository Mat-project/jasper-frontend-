...
export default function DrawingCategoriesPage() {
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
    ...
    mockService.createDrawingCategory(form.code, form.name);
    setIsAddOpen(false);
    loadData();
  };
  ...
  const handleEditSubmit = (e: React.FormEvent) => {
    ...
    mockService.updateDrawingCategory(selectedCat.id, form.code, form.name);
    setIsEditOpen(false);
    loadData();
  };

  const handleDelete = (id: string) => {
    if (confirm("Are you sure you want to delete this drawing category?")) {
      mockService.deleteDrawingCategory(id);
      loadData();
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage categorization codes for design sheets, revisions, and models.
          </p>
        </div>
        <button
            ...
            {categories.length === 0 ? (
              <tr>
                <td colSpan={3} className="px-6 py-12 text-center text-muted-foreground">
                  No drawing categories created yet.
                </td>
              </tr>
            ) : (
              categories.map((cat) => (
      ...
      <Dialog isOpen={isAddOpen} onClose={() => setIsAddOpen(false)} title="Add Category" size="sm">
      ...
      <Dialog isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} title="Edit Category" size="sm">
